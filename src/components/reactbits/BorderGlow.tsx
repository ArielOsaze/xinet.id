...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  className = '',
  edgeSensitivity = 30,
  // Stock default is '40 80 80' — HSL hue 40, i.e. amber. That produced a
  // yellow glow that clashed with the cool brand palette. Default is now the
  // brand cyan (hue 187) so a caller that forgets to pass a colour still gets
  // something on-palette.
  glowColor = '187 82 74',
  ...
  const borderOpacity = isVisible
    ...
    : 0;
  // Eased so the glow is already clearly present at mid-card, not only when the
  // pointer is millimetres from the border. The stock linear ramp made the
  // effect nearly invisible in normal use.
  const glowOpacity = isVisible
    ? Math.min(1, Math.max(0, (edgeProximity * 100 - edgeSensitivity) / (100 - edgeSensitivity)) * 1.35)
    : 0;
